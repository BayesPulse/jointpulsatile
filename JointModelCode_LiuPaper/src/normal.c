     ...
     char *c;
{
  ...
  if (ier < 32)
    printf(" *** I M S L (UERTST) *** NON-DEFINED  %s   %d  (ier = %d)\n",c,ier-0,ier);
  else if (ier < 64)
    printf(" *** I M S L (UERTST) *** WARNING  %s   %d  (ier = %d)\n",c,ier-32,ier);
  else if (ier < 128)
    printf(" *** I M S L (UERTST) *** WARNING(with fix)  %s   %d  (ier = %d)\n",c,ier-64,ier);    
  else
    printf(" *** I M S L (UERTST) *** TERMINAL  %s   %d  (ier = %d)\n",c,ier-128,ier);    
}
...
double prob_std_norm(double z,double *dens)
{
  ...
  prob = 1.0 - (*dens)*t*((((1.330274*t-1.821256)*t+1.781478)*t-0.3565638)*t+0.3193815);
